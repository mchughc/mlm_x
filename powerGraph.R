...
powerGraph <- function(totalRes,b,fn,xlims=c(0,1),values=FALSE){
  ...
  if(!values){
    ...
    dev.off()
    ret <- NULL
  }else{ # return values and not plot
    ret <- data.frame("x_fp"=fp_x,"x_tp"=tp_x,"xiter"=iterX,
                      "auto_fp"=fp_auto,"auto_tp"=tp_auto,"autoiter"=iterAuto,
                      "both_fp"=fp_xauto,"both_tp"=tp_xauto,"bothiter"=iterBoth)
  }
