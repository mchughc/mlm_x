...
oneSim <- function(kinFull,kinFullX,sigmaA,sigmaX,sigmaE,herit,p,i,fn){
  ...
  # loop through backwards to get more iterations of the lower elements in the list
  for(i in length(sigmaE):1){
    ...
    varComps <- data.frame(varCompInclX$varComp)
    varCompsExclX <- data.frame(varCompExclX$varComp)
    simRes <- list(mmResX,mmRes,metrics,varComps,varCompsExclX)
    names(simRes) <- c("mmRes_adjForX","mmRes_notAdjForX","metricsSet","varComps_est_inclX",
                       "varComps_est_exclX")
