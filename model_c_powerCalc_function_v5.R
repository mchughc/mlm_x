...
Family_alleles_NmarkerX<-function(FREQ1,nloci,SEX)
  {
      GENO= matrix(0,nloci,16);
        ...
        GENO[,1]=INDIV <- alleles <- Nmarker(FREQ1,nloci)
        GENO[,3]=INDIV <- alleles <- Nmarker(FREQ1,nloci)
        GENO[,2]=INDIV <- alleles <- NmarkerX(FREQ1,nloci)
        GENO[,4]=INDIV <- alleles <- NmarkerX(FREQ1,nloci)
        GENO[,5]=INDIV <- alleles <- NmarkerX(FREQ1,nloci)
...
#  kinX <- matrix(kinX,nrow=n,ncol=n)
  ...
  randomEffects <- get(load("randomEffects.RData"))
  ...
  males <- SEX=="M"
  simGenos <- lapply(simGenos,function(x){x[,males] <- 4*x[,males];x[,!males] <- 2*x[,!males]; return(x)})
  # simGenos is a list of length num pedigrees (500), each entry is 1x16

  geno <- matrix(NA,nrow=10,ncol=n)
